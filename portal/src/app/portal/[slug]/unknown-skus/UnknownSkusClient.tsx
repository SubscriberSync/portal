...
export default function UnknownSkusClient({
  ...
  const handleDeleteAlias = async (sku: string) => {
    ...
    try {
      ...
      // Note: We don't add it back to unknownSkus since we'd need to re-fetch the count
    } catch (err) {
      ...
      {mappingModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#1a1a1a] rounded-2xl border border-[rgba(255,255,255,0.1)] p-6 w-full max-w-md shadow-2xl">
            ...
            <button
              onClick={handleCloseMapper}
              className="absolute top-4 right-4 p-2 rounded-lg text-[#71717a] hover:text-white hover:bg-[rgba(255,255,255,0.05)] transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="mb-4">
              ...
              />
              <p className="text-xs text-[#52525b] mt-1">
                Enter the box number this SKU represents (1 = first box, 2 = second, etc.)
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-3 rounded-lg bg-[#e07a42] text-white font-semibold hover:bg-[#c86a35] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
                  <>
